resource "aws_codedeploy_app" "admin" {
  compute_platform = "ECS"
  name             = "${local.prefix}-admin"
}

resource "aws_codedeploy_deployment_group" "admin" {
  app_name               = aws_codedeploy_app.admin.name
  deployment_group_name  = "${local.prefix}-admin"
  service_role_arn       = aws_iam_role.codedeploy.arn
  deployment_config_name = "CodeDeployDefault.ECSAllAtOnce"

  blue_green_deployment_config {
    deployment_ready_option {
      action_on_timeout = "CONTINUE_DEPLOYMENT"
    }

    terminate_blue_instances_on_deployment_success {
      action                           = "TERMINATE"
      termination_wait_time_in_minutes = 1
    }
  }

  deployment_style {
    deployment_option = "WITH_TRAFFIC_CONTROL"
    deployment_type   = "BLUE_GREEN"
  }

  ecs_service {
    cluster_name = aws_ecs_cluster.main.name
    service_name = aws_ecs_service.admin.name
  }

  load_balancer_info {
    target_group_pair_info {
      prod_traffic_route {
        listener_arns = [aws_lb_listener.admin_https.arn]
      }

      target_group {
        name = aws_lb_target_group.admin["blue"].name
      }

      target_group {
        name = aws_lb_target_group.admin["green"].name
      }
    }
  }
}
